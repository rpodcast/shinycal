...
```{r apply_date_format}
time_parser <- function(x, zone, format = "%Y%m%dT%H%M%S") {
  as.character(clock::date_time_parse(x, zone, format = format))
}

dt_df2 <- dt_df %>%
  mutate(
    timezone = stringr::str_remove_all(timezone, "TZID="),
    start = purrr::map2_chr(DTSTART, timezone, ~time_parser(.x, .y)),
    end =  purrr::map2_chr(DTEND, timezone, ~time_parser(.x, .y))) %>%
  select(uid, start, end, timezone)
...
```{r final_polish}
final_df <- raw_df %>%
  select(uid, title = SUMMARY, body = DESCRIPTION, location = LOCATION) %>%
